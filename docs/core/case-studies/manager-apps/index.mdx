...
by ensuring a consistent [page layout](/core-patterns/layout/page-layouts) and [max width](/core-patterns/layout/grid#content-max-width) as well as a consistent type
hierarchy across apps, using the Forge [typography](/core-components/typography/guidance) styles. Controls such as primary actions, a
[local search](/patterns/search-filter#1-initiate-search), and [pagination](/components/paginator) were all placed in consistent locations across apps. 
...
pages. A "chevron_left" icon button navigates to a child page;  the [upward navigation](/core-patterns/navigation/hierarchical#2-upward-navigation) pattern navigates
...
Since the apps are meant to be a suite, the team used the [app launcher](/components/omnibar-app-launcher) to allow for navigation between apps. 
...
Although not a Forge pattern, the team used concepts from Forge, such as Forge [colors](/core-components/color/guidance#4-informational-colors) and principles from the
[system communication](/core-patterns/system-communication/#2-user-goal-ask-me) pattern to develop a pattern to meet their needs. 
...
The stepper is displayed in its own focused dialog, using concepts from [modality](/core-patterns/modality) to indicate a closed process with beginning an end. A
...
- [page layout](/core-patterns/layout/page-layouts)
- [max width](/core-patterns/layout/grid#content-max-width)
- [system communication](/core-patterns/system-communication)
- [local search](/patterns/search-filter#1-initiate-search)
- [typography](/core-components/typography/guidance)
- [upward navigation](/core-patterns/navigation/hierarchical#2-upward-navigation)
