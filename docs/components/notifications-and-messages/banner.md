---
description: A banner displays a prominent message and related optional actions. It is dismissible.
keywords: ['banner', 'dismissible', 'announcement', 'alert', 'dismiss']
---
...
<ComponentVisual
  ...
  storybookUrl="https://forge.tylerdev.io/main/?path=/story/components-banner--default" />
...
- Displaying high priority information where action is needed from a user, such as a server error or submission failure. Use a [dialog](/components/dialog) instead. 
- Displaying reactive or confirming information such as a confirmation of a save, delete, or add. Use a [toast](/components/toast) instead. 
- Don't use to confirm whether a user would like to proceed. Use a [dialog](/components/dialog) instead. 
...
![Anatomy of the banner.](/img/components/banner/page-banner-parts.png)
...
4. **Action.** (Optional) "Learn more" by default; text may be customized depending on the message or business requirements. By default, the action is intended to launch a simple or complex [dialog](/components/dialog) with additional information, but the action may link to a new page instead if appropriate. 
...
The banner displays preview text; users may click the banner for additional information. The action may launch a [dialog](/components/dialog) that can include text, illustrations, or media. The banner may be dismissed via the dismiss icon or by interacting with and closing the dialog. Once the banner has been interacted with, it is not shown again for the same user. 
...
![ The mobile banner resizes to display on mobile. ](/img/components/banner/mobile-banner.png)
...
- Use a [dialog](/components/dialog) for high priority alerts that require user action.
